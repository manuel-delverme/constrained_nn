...
def train(model, device, train_loader, optimizer, epoch, step, adversarial, aux_optimizer=None):
    config.tb.run.alert("log", f"{inspect.currentframe().f_lineno}", wait_duration=0)
    model.train()

    for batch_idx, (data, target, indices) in enumerate(train_loader):
        config.tb.run.alert("log", f"{inspect.currentframe().f_lineno} {batch_idx}", wait_duration=0)
