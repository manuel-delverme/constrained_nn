        ...
        if adversarial:
            ...
            for idx, (defect, rh, xi) in enumerate(zip(defects, rhs, model.state_net.states)):
                ...
                if config.distributional:
                    config.tb.add_histogram(f"train/h{idx}/mean_scale", xi.scale.mean(axis=1).cpu().detach().numpy(), batch_idx + step)
                    config.tb.add_scalar(f"train/h{idx}/mean_mean", xi.mean.mean(), batch_idx + step)
                    config.tb.add_histogram(f"train/h{idx}/mean_pdist", torch.pdist(xi.mean).cpu().detach().numpy(), batch_idx + step)
