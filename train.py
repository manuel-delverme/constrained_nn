        ...
        if adversarial:
            ...
            for idx, (defect, rh, xi) in enumerate(zip(defects, rhs, model.state_net.states)):
                ...
                if config.distributional:
                    config.tb.add_histogram(f"h{idx}/train/state_scale_mean", xi.scale.mean(axis=1).cpu().detach().numpy(), batch_idx + step)
                    config.tb.add_scalar(f"h{idx}/train/state_loc_mean", xi.mean.mean(), batch_idx + step)
                    config.tb.add_histogram(f"h{idx}/train/state_mean_pdist", torch.pdist(xi.mean).cpu().detach().numpy(), batch_idx + step)
