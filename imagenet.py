...
def main(tb, args, task_config):
    ...
    if "SLURM_JOB_ID" in os.environ.keys():
        dataset_path = args.dataset_path.format(args.dataset, "ImageNet")
    else:
    ...
    if config.evaluate:
        validate(tb, val_loader, model, criterion, task_config)
        return
    ...
    for epoch in range(task_config.start_epoch, task_config.epochs):
        ...
        # train for one epoch
        train(tb, train_loader, model, criterion, optimizer, epoch, task_config)

        # evaluate on validation set
        acc1 = validate(tb, val_loader, model, criterion, task_config)
...
def train(tb, train_loader, model, criterion, optimizer, epoch, args):
...
def validate(tb, val_loader, model: torchvision.models.AlexNet, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
