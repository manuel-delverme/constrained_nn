...
def load_imagenet():
    if "SLURM_JOB_ID" in os.environ.keys():
        ...
        os.system(f"mkdir -p {dataset_home}/train {dataset_home}/val/ > /dev/null")
        os.system(f"tar -xvf /network/datasets/imagenet/ILSVRC2012_img_train.tar -C {dataset_home}/train/ > /dev/null")
        os.system(f"tar -xvf /network/datasets/imagenet/ILSVRC2012_img_val.tar -C {dataset_home}/val/ > /dev/null")
        os.system(
            'find ' + dataset_home + '/train/ -name "*.tar" | while read NAME ; do mkdir -p "${NAME%.tar}"; tar -xvf "${NAME}" -C "${NAME%.tar}"; rm -f "${NAME}"; done > /dev/null'
        )

        os.system(f'cp -r /network/datasets/imagenet.var/imagenet_torchvision/val {dataset_home}/ > /dev/null')
        print('Imagenet copied.')
    else:
