...
def load_imagenet():
    if "SLURM_JOB_ID" in os.environ.keys():
        ...
        print(f"tar -xvf /network/datasets/imagenet/ILSVRC2012_img_val.tar -C {dataset_home}/val/")
        ...
        # print('find ' + dataset_home + '/val/ -name "*.tar" | while read NAME ; do mkdir -p "${NAME%.tar}"; tar -xvf "${NAME}" -C "${NAME%.tar}"; rm -f "${NAME}"; done')
        # os.system('find ' + dataset_home + '/val/ -name "*.tar" | while read NAME ; do mkdir -p "${NAME%.tar}"; tar -xvf "${NAME}" -C "${NAME%.tar}"; rm -f "${NAME}"; done')

        start = time.time()
        os.system(f'cp -r /network/datasets/imagenet.var/imagenet_torchvision/val {dataset_home}/val/')
        print("time to copy validation", time.time() - start)
    else:
