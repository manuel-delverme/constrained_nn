...
class TargetPropNetwork(nn.Module):
    def __init__(self, train_loader=None, multi_stage=True):
        ...
        if multi_stage:
            if config.distributional:
                ...
                self.x1 = torch.distributions.Normal(nn.Parameter(torch.zeros(num_classes, 128)), 1.)
    ...
    def forward(self, x0, indices):
        ...
        if config.distributional:
            ...
            dist = self.x1.mean[targets] - x1_hat
            h = 0.5 * (1 + torch.erf(dist * self.x1.scale[targets].reciprocal() / math.sqrt(2)))
            samples = self.x1.sample((x0.shape[0],))
            x1 = []
