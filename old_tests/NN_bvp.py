...
def run_experiment(num_outputs, trainX, trainY, testX, testY, iters):
    ...
    for idx, (block, x_t, y_t) in reversed(list(enumerate(zip(model.blocks, block_ins, block_outs)))):
        ...
        def equality_constraints(_block):
            defect = _block(x_t) - y_t
            # defect = np.minimum(defect, 10e-4)
        ...
        print("pre", equality_constraints(get_x(initial_values)))
...
def main():
    ...
    for _ in tqdm.trange(1):
        model = run_experiment(num_outputs, train_x, train_y, test_x, test_y, iters=config.optimization_iters)
        print(model)
